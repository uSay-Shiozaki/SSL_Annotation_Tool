...
parser.add_argument("--dump_checkpoints", type=str, default="./checkpoints")
...
def main():
    ...
    # build the queue
    queue = None
    queue_path = os.path.join(params.dump_path, "queue" + str(params.rank) + ".pth")
    if os.path.isfile(queue_path):
        queue = torch.load(queue_path)["queue"]
    # the queue needs to be divisible by the batch size
    params.queue_length -= params.queue_length % (params.batch_size * params.world_size)
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        if params.queue_length > 0 and epoch >= params.epoch_queue_starts and queue is None:
            queue = torch.zeros(
                len(params.crops_for_assign),
                params.queue_length // params.world_size,
                params.feat_dim,
            ).cuda()
...
def distributed_sinkhorn(out):
    ...
    B = Q.shape[1] * params.world_size # number of samples to assign
