...
def main_eval(
    ...
    if "swin" in arch:
        ...
        model = models.__dict__[arch](
            window_size=window_size, patch_size=patch_size, num_classes=0
        )
        embed_dim = model.num_features
    else:
        model = models.__dict__[arch](patch_size=patch_size, num_classes=0)
...
def kmeans_eval(
    ...
    with torch.no_grad():
        ...
        if not k_means:
            ...
            kmeans_pred = KMeans(n_clusters=n_clusters, random_state=251).fit_predict(
                output_labels
            )
            ...
            f.write(
                f"k_means score with {arch} {patch_size}x{patch_size} -> NMI, ARI, FMI = {output_scores}\n"
            )
...
def get_true_pred_map(label, pred):
    ...
    for i in range(len(df)):
        ax.bar(df.columns, df.iloc[i], bottom=df.iloc[:i].sum(), alpha=0.7)
